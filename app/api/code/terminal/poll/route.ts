...
import { validateTerminalPollRequest } from "@/features/code-lab/code-terminal-request";
...
export async function POST(request: Request) {
  ...
  if (!body) return apiError("invalid_request", "A valid poll request is required.", 400);

  const parsed = validateTerminalPollRequest(body);
  ...
  try {
    const result = await selection.adapter.poll({
    ...
    return apiError("terminal_error", "The terminal service failed to return output.", 502);
  }
}

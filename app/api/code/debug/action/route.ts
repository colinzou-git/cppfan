...
import { validateDebugActionRequest } from "@/features/code-lab/code-debug-request";
...
export async function POST(request: Request) {
  ...
  const parsed = validateDebugActionRequest(body);
  if (!parsed.ok) return apiError(parsed.code, parsed.message, 400);

  const selection = selectDebugger();
  if (selection.kind === "unconfigured") {
    return NextResponse.json({ result: unconfiguredSnapshot([], selection.note) }, { headers: NO_STORE });
  }

  try {
    const result = await selection.adapter.action({
    ...
    return apiError("debugger_error", "The debugger service failed to run that action.", 502);
  }
}

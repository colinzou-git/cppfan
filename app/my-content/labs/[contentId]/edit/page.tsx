...
import { LabEditor } from "@/features/user-content/lab-editor";
import { getContentVersions, getLabForOwner } from "@/features/user-content/user-content-queries";
import { requireOwnerSession } from "@/features/user-content/require-owner";

export const dynamic = "force-dynamic";

export default async function EditLabPage({ params }: { params: Promise<{ contentId: string }> }) {
  const { contentId } = await params;
  await requireOwnerSession(`/my-content/labs/${contentId}/edit`);
  ...
  return (
    <PageShell className="grid gap-6" size="reading">
      <header className="rounded-3xl border border-white/70 bg-white/85 p-5 shadow-sm">
        ...
        <h1 className="mt-2 text-3xl font-black text-slate-950">Edit lab</h1>

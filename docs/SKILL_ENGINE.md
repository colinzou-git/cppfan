...
are compared to actual results and emit skill-event _drafts_ only (no mastery
...
- An explanatory lesson explicitly completed with Hard, Good, or Mastered
- A quiz question
...
- Starting a lesson
- Explicitly self-assessing a completed lesson
- Viewing a concept
- Answering a quiz
- Using a hint
- Completing a review
...
**Cross-context coverage (#417).** Mastery also accounts for _evidence diversity_.
...
## Lesson acquisition and review ratings

An active lesson is completed only when the learner explicitly chooses Hard,
Good, or Mastered at the end of the lesson. That trusted mutation creates and
rates the exact lesson's canonical FSRS card, appends its review log, and writes
`lesson_self_assessed` evidence atomically. Merely opening, scrolling, or
leaving the lesson writes no completion evidence.

Initial lesson choices map to FSRS Hard, Good, and Easy respectively. The UI
label `Mastered` means FSRS Easy only: it is neither permanent nor broader skill
mastery. When the lesson later becomes due, its recall-first review presents
Again, Hard, Good, and Mastered; Again remains available for lapse/relearning.
Lesson review content is hidden until reveal so the explanation does not answer
its own recall prompt.

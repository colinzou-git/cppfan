create or replace function public.cancel_study_goal(
  p_goal_id uuid,
...
begin
  ...
  v_fingerprint := md5(concat_ws(':', 'cancel', p_goal_id::text, p_expected_revision::text, coalesce(p_reason, '')));
  ...
  if found then
    if v_receipt.mutation_kind <> 'cancel' or v_receipt.request_fingerprint <> v_fingerprint then
  ...
  select v_user_id, p_goal_id, r.id, 'cancelled', nullif(btrim(p_reason), ''),
  ...
  insert into public.study_goal_mutation_receipts (
    ...
    v_user_id, p_submission_id, 'cancel', v_fingerprint, p_goal_id, v_revision
  );
...
revoke all on function public.cancel_study_goal(uuid, integer, uuid, text) from public;
grant execute on function public.cancel_study_goal(uuid, integer, uuid, text) to authenticated;

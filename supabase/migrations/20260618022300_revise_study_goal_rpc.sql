create or replace function public.revise_study_goal(
  p_goal_id uuid,
...
begin
  ...
  v_fingerprint := md5(jsonb_build_object(
    'mutation', 'revise', 'goal', p_goal_id,
    'expected_revision', p_expected_revision, 'title', btrim(p_title),
  ...
  if found then
    if v_receipt.mutation_kind <> 'revise' or v_receipt.request_fingerprint <> v_fingerprint then
  ...
  select g.current_revision, g.status
  ...
  if v_current_revision <> p_expected_revision then raise exception 'stale_goal_revision'; end if;

  v_new_revision := v_current_revision + 1;

  update public.study_goals
  set title = btrim(p_title), current_revision = v_new_revision, updated_at = now()
  where id = p_goal_id and user_id = v_user_id;

  v_revision_id := public.study_goal_insert_revision(
    v_user_id, p_goal_id, v_new_revision,
  ...
  insert into public.study_goal_events (
    ...
    v_user_id, p_goal_id, v_revision_id, 'revised',
    nullif(btrim(p_recommendation_reason), ''),
    jsonb_build_object('prior_revision', v_current_revision, 'revision_number', v_new_revision)
  );
  ...
  insert into public.study_goal_mutation_receipts (
    ...
    v_user_id, p_submission_id, 'revise', v_fingerprint, p_goal_id, v_new_revision
  );

  return query select p_goal_id, v_new_revision, false;
exception
  when unique_violation then
    select r.* into v_receipt
    from public.study_goal_mutation_receipts r
    where r.user_id = v_user_id and r.submission_id = p_submission_id;
    if found and v_receipt.mutation_kind = 'revise' and v_receipt.request_fingerprint = v_fingerprint then
      return query select v_receipt.goal_id, v_receipt.revision_number, true;
      return;
    end if;
    raise;
...
revoke all on function public.revise_study_goal(uuid, integer, uuid, text, date, date, text, text, text, text, text, jsonb) from public;
grant execute on function public.revise_study_goal(uuid, integer, uuid, text, date, date, text, text, text, text, text, jsonb) to authenticated;

-- #488 Phase 2: allow publishing kind = 'exercise'.
--
-- Extends publish_user_content so an exercise projects the same owner-scoped
-- skill + learning item as a lesson (so it participates in mastery/skill-map/
-- goals/recommendations), using the exercise payload's prompt (not content) and
-- a worked_example item type. The lesson-only interactive child projections
-- (choices/parsons/completion/review cards) run only for kind = 'lesson'. Code
-- Lab execution for a published exercise is resolved dynamically from the
-- payload in the app layer (a later slice). Based on the review-cards version of
-- the function; the lesson behavior is unchanged.
...
begin
  ...
  if v_item.kind not in ('lesson', 'exercise') then
    raise exception 'only lessons and exercises can be published in this phase' using errcode = '22023';
  end if;

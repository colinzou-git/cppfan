-- #490 Phase 4: allow publishing kind = 'interview_problem'.
--
-- Extends publish_user_content so an interview problem projects the same
-- owner-scoped skill + worked_example learning item as an exercise/lab
-- (participating in mastery/skill-map/goals/recommendations/readiness), using the
-- interview payload's statement as the prompt. Judge/AI evaluation and session
-- workflows resolve dynamically from the payload in the app layer (later slices).
-- Based on the lab version of the function; lesson/exercise/lab behavior is
-- unchanged.
...
begin
  ...
  if v_item.kind not in ('lesson', 'exercise', 'lab', 'interview_problem') then
    raise exception 'unsupported content kind for publication' using errcode = '22023';
  end if;
  ...
  if v_item.kind = 'exercise' then
    ...
    v_explanation := v_payload->>'solutionExplanation';
  elsif v_item.kind = 'interview_problem' then
    v_item_type := 'worked_example';
    v_prompt := coalesce(nullif(v_payload->>'statement', ''), v_title);
    v_explanation := v_payload->>'solutionExplanation';
  else
  ...
  -- Interview problems use easy/medium/hard; map onto the skills level vocabulary.

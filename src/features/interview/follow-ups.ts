...
export const followUps: FollowUp[] = [
  ...
  {
    id: "fu.lis.russian-dolls",
    version: 1,
    problemId: "iv.dp.longest-increasing-subsequence",
    parentVersion: 1,
    kind: "compare_approaches",
    trigger: "base_correct",
    timing: "after_acceptance",
    priority: 1,
    prompt: "Sort pairs so equal widths cannot chain, then reduce height nesting to LIS. Explain the tie-order invariant before coding.",
    affectedConstraints: "A two-dimensional nesting constraint replaces a one-dimensional sequence.",
    expectedReasoningShift: "Sort width ascending and equal-width height descending, then apply strict LIS to heights.",
    targetRubricDimensions: ["optimization","follow_up_adaptability"],
    timeBudgetMinutes: 9,
    revealPolicy: "after_attempt",
    explanation: "Russian-doll nesting becomes LIS only after the equal-width tie rule prevents invalid chains."
  },
  {
    id: "fu.trapped-water.2d",
    version: 1,
    problemId: "iv.twoptr.trapped-water",
    parentVersion: 1,
    kind: "compare_approaches",
    trigger: "base_correct",
    timing: "after_acceptance",
    priority: 1,
    prompt: "Heights now form a 2-D grid. Why do two left/right pointers stop being sufficient, and what frontier structure replaces them?",
    affectedConstraints: "Containment depends on the lowest boundary around a region, not two linear sides.",
    expectedReasoningShift: "Expand inward from the outer boundary with a min-heap, carrying the best enclosing boundary height.",
    targetRubricDimensions: ["complexity","follow_up_adaptability"],
    timeBudgetMinutes: 9,
    revealPolicy: "after_attempt",
    explanation: "The 2-D invariant is a minimum boundary frontier, so a heap-based flood replaces the linear two-pointer invariant."
  },
  {
    id: "fu.k-stop.return-route",
    version: 1,
    problemId: "iv.graph.k-stop-cheapest-route",
    parentVersion: 1,
    kind: "return_witness",
    trigger: "base_correct",
    timing: "after_acceptance",
    priority: 1,
    prompt: "Return the actual cheapest route as well as its price while preserving the stop bound.",
    affectedConstraints: "The DP/state search must retain enough predecessor information to reconstruct a bounded-edge witness.",
    expectedReasoningShift: "Track predecessor by layer/state and backtrack from the best destination layer.",
    targetRubricDimensions: ["correctness","follow_up_adaptability"],
    timeBudgetMinutes: 8,
    revealPolicy: "after_attempt",
    explanation: "A price alone discards path identity; layered parent pointers reconstruct the chosen bounded route."
  },
  {
    id: "fu.alien.stable-order",
    version: 1,
    problemId: "iv.graph.alien-order",
    parentVersion: 1,
    kind: "stable_ordering",
    trigger: "base_correct",
    timing: "after_acceptance",
    priority: 1,
    prompt: "When several alphabet orders are valid, guarantee the lexicographically smallest valid order.",
    affectedConstraints: "Topological sorting needs deterministic tie-breaking among zero-indegree symbols.",
    expectedReasoningShift: "Use a min-heap (or ordered set) for the available zero-indegree frontier.",
    targetRubricDimensions: ["correctness","communication"],
    timeBudgetMinutes: 6,
    revealPolicy: "after_attempt",
    explanation: "Kahn's algorithm plus an ordered frontier selects the smallest available symbol at every step."
  },
  {
    id: "fu.multi-source.weighted-delay",
    version: 1,
    problemId: "iv.graph.multi-source-spread",
    parentVersion: 1,
    kind: "compare_approaches",
    trigger: "base_correct",
    timing: "after_acceptance",
    priority: 1,
    prompt: "Each cell now has a non-negative activation delay. When does plain multi-source BFS become incorrect, and what replaces it?",
    affectedConstraints: "Edges no longer have uniform cost, so BFS layer number is not minimum elapsed time.",
    expectedReasoningShift: "Start all sources at distance zero and use multi-source Dijkstra; 0/1 delays admit a deque optimization.",
    targetRubricDimensions: ["complexity","follow_up_adaptability"],
    timeBudgetMinutes: 8,
    revealPolicy: "after_attempt",
    explanation: "Weighted propagation is a shortest-path problem; the frontier must be ordered by elapsed cost rather than hop count."
  },
  {
    id: "fu.serialize.general-graph",
    version: 1,
    problemId: "iv.tree.serialize-roundtrip",
    parentVersion: 1,
    kind: "compare_approaches",
    trigger: "base_correct",
    timing: "after_acceptance",
    priority: 1,
    prompt: "Generalize serialization from a tree to a graph that may have cycles and shared nodes. What identity information becomes necessary?",
    affectedConstraints: "Tree structure has one parent path per node; a graph can revisit the same object through multiple edges.",
    expectedReasoningShift: "Assign stable node IDs, serialize each node once, and encode edges by ID while tracking visited nodes.",
    targetRubricDimensions: ["communication","follow_up_adaptability"],
    timeBudgetMinutes: 9,
    revealPolicy: "after_attempt",
    explanation: "Graph serialization needs explicit identity and a visited set so cycles and shared references survive the round trip."
  },
  {
    id: "fu.edit-distance.edit-script",
    version: 1,
    problemId: "iv.dp.edit-distance",
    parentVersion: 1,
    kind: "return_witness",
    trigger: "base_correct",
    timing: "after_acceptance",
    priority: 1,
    prompt: "Return one minimum edit script, not only the edit count.",
    affectedConstraints: "Rolling-row optimization discards the decisions needed for reconstruction.",
    expectedReasoningShift: "Keep the full DP or explicit parent choices, then backtrack from (n,m) to emit insert/delete/replace operations.",
    targetRubricDimensions: ["correctness","follow_up_adaptability"],
    timeBudgetMinutes: 9,
    revealPolicy: "after_attempt",
    explanation: "Witness reconstruction needs retained decisions (or recomputation) rather than only the final scalar distance."
  },
  {
    id: "fu.trie.delete-prefix-count",
    version: 1,
    problemId: "iv.trie.prefix-index",
    parentVersion: 1,
    kind: "support_updates",
    trigger: "base_correct",
    timing: "after_acceptance",
    priority: 1,
    prompt: "Add deletion plus prefixCount(prefix). Nodes may remain allocated, but counts must stay correct after duplicates and deletes.",
    affectedConstraints: "Presence alone is insufficient; updates require multiplicity/count state along every prefix path.",
    expectedReasoningShift: "Maintain terminal multiplicity and subtree/prefix counts, decrementing only for a word that is actually present.",
    targetRubricDimensions: ["correctness","optimization"],
    timeBudgetMinutes: 9,
    revealPolicy: "after_attempt",
    explanation: "Reference counts along trie paths support deletion and prefix counts without scanning descendants."
  },
  {
    id: "fu.min-rooms.delayed-most-used",
    version: 1,
    problemId: "iv.intervals.min-rooms",
    parentVersion: 1,
    kind: "operation_mix_change",
    ...
    timing: "after_acceptance",
    priority: 1,
    prompt: "There are a fixed number of rooms. If all are occupied, delay a meeting until the earliest room frees; return the room used most often, breaking ties by smaller id.",
    affectedConstraints: "The task changes from counting overlaps to simulating resource assignment and delayed starts.",
    expectedReasoningShift: "Use one min-heap for free room IDs and another for busy rooms ordered by (endTime, roomId).",
    targetRubricDimensions: ["optimization","follow_up_adaptability"],
    timeBudgetMinutes: 10,
    revealPolicy: "after_attempt",
    explanation: "Two heaps separate resource availability from next completion; delayed meetings preserve duration when rescheduled."
  },
  {
    id: "fu.cheapest-route.timetable",
    version: 1,
    problemId: "iv.graph.cheapest-route",
    parentVersion: 1,
    kind: "operation_mix_change",
    ...
    timing: "after_acceptance",
    priority: 1,
    prompt: "Edges are timetable departures rather than always-available static costs. Explain what state must include and why ordinary static Dijkstra edges are insufficient.",
    affectedConstraints: "The cost/availability of an edge depends on arrival time at its source.",
    expectedReasoningShift: "Relax the next feasible departure after the current arrival time; model time-dependent transitions and verify FIFO assumptions before using Dijkstra.",
    targetRubricDimensions: ["complexity","communication"],
    timeBudgetMinutes: 9,
    revealPolicy: "after_attempt",
    explanation: "Time-dependent routing requires relaxation based on arrival time; static edge weights no longer describe the transition."
  },
  {
    id: "fu.tree-max-path.return-witness",
    version: 1,
    problemId: "iv.tree.max-path-sum",
    parentVersion: 1,
    kind: "return_witness",
    trigger: "base_correct",
    timing: "after_acceptance",
    priority: 1,
    prompt: "Return the node-value sequence for one maximum-sum path, not only the sum.",
    affectedConstraints: "The postorder DP must preserve which downward child produced each contribution and where the global best crossed.",
    expectedReasoningShift: "Store best-child choices and the node where left+node+right sets the global optimum, then reconstruct both arms.",
    targetRubricDimensions: ["correctness","follow_up_adaptability"],
    timeBudgetMinutes: 8,
    revealPolicy: "after_attempt",
    explanation: "Path reconstruction augments the scalar tree DP with chosen-child pointers and a remembered best crossing node."
  },
  {
    id: "fu.time-key-value.compaction",
    version: 1,
    problemId: "iv.design.time-key-value",
    parentVersion: 1,
    kind: "out_of_memory",
    trigger: "base_correct",
    timing: "after_acceptance",
    priority: 1,
    prompt: "History is too large to keep every version forever. Design bounded retention while preserving exact reads for the retained time window.",
    affectedConstraints: "The append-only per-key vectors grow without bound and retention semantics become part of correctness.",
    expectedReasoningShift: "Define a retention watermark, compact versions older than it while keeping the latest predecessor needed at the boundary, and discuss amortized cleanup.",
    targetRubricDimensions: ["optimization","communication"],
    timeBudgetMinutes: 9,
    revealPolicy: "after_attempt",
    explanation: "Windowed retention can compact history safely if each key retains the value effective at the retention boundary plus newer versions."
  }
];

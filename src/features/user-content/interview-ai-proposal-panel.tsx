...
import {
  ...
  type InterviewAuthoringProposal,
  type IdentifiedInterviewOperation
} from "./interview-ai-authoring";

type PanelStatus = "idle" | "loading" | "ready" | "error";

/**
 * AI authoring assistant panel for interview problems (#490). Asks
 * /api/ai/author (which branches to the interview service) for a structured
 ...
 */
export function InterviewAiProposalPanel({
  contentId,
  onApply
}: {
  contentId?: string;
  onApply: (operations: InterviewAuthoringOperation[]) => void;
}) {
  const [instruction, setInstruction] = useState("");
  const [proposal, setProposal] = useState<InterviewAuthoringProposal | null>(null);
  ...
  async function ask() {
    if (!contentId) {
      setStatus("error");
      setMessage("Save the problem first so the assistant can read it.");
      return;
    }
    ...
    try {
      ...
      const data = (await res.json()) as { proposal?: InterviewAuthoringProposal };
  ...
  return (
    <div className="grid gap-3 rounded-2xl border border-violet-200 bg-violet-50/60 p-4" data-testid="interview-ai-panel">
      ...
      <textarea
        ...
        placeholder="e.g. Turn this idea into a full problem with a hint ladder and two tests"
      />
      ...
      {proposal ? (
        <div className="grid gap-2">
          ...
          <ul className="grid gap-1">
            {(proposal.operations as IdentifiedInterviewOperation[]).map((op) => (
              <li key={op.id} className="flex items-start gap-2 text-sm text-slate-800">
                <input
                  ...
                  aria-label={describeInterviewOperation(op)}
                />
                <span>{describeInterviewOperation(op)}</span>
              </li>
            ))}
          </ul>

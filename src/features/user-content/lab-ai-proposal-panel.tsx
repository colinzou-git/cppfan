...
import {
  ...
  type LabAuthoringProposal,
  type IdentifiedLabOperation
} from "./lab-ai-authoring";

type PanelStatus = "idle" | "loading" | "ready" | "error";

/**
 * AI authoring assistant panel for labs (#489). Asks /api/ai/author (which
 * branches to the lab service) for a structured proposal, lists the operations
 * with per-item accept toggles, and hands the accepted ones back to the editor —
 * it never overwrites the draft directly.
 */
export function LabAiProposalPanel({
  contentId,
  onApply
}: {
  contentId?: string;
  onApply: (operations: LabAuthoringOperation[]) => void;
}) {
  const [instruction, setInstruction] = useState("");
  const [proposal, setProposal] = useState<LabAuthoringProposal | null>(null);
  ...
  async function ask() {
    if (!contentId) {
      setStatus("error");
      setMessage("Save the lab first so the assistant can read it.");
      return;
    }
    ...
    try {
      ...
      const data = (await res.json()) as { proposal?: LabAuthoringProposal };
  ...
  return (
    <div className="grid gap-3 rounded-2xl border border-violet-200 bg-violet-50/60 p-4" data-testid="lab-ai-panel">
      ...
      <textarea
        ...
        placeholder="e.g. Turn this into three milestones and add a test for each"
      />
      ...
      {proposal ? (
        <div className="grid gap-2">
          ...
          <ul className="grid gap-1">
            {(proposal.operations as IdentifiedLabOperation[]).map((op) => (
              <li key={op.id} className="flex items-start gap-2 text-sm text-slate-800">
                <input
                  ...
                  aria-label={describeLabOperation(op)}
                />
                <span>{describeLabOperation(op)}</span>
              </li>
            ))}
          </ul>

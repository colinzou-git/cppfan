...
const output = process.argv[2] ?? ".ai/code-index.json";
...
function symbolsFrom(sourceFile, path) {
  const symbols = [];
  for (const node of sourceFile.statements) {
    ...
    symbols.push({ name: node.name.getText(sourceFile), kind, path, line, exported: exported(node) });
  }
  return symbols;
}

function importsFrom(sourceFile, path) {
  const imports = [];
  for (const node of sourceFile.statements) {
    if (!ts.isImportDeclaration(node) || !ts.isStringLiteral(node.moduleSpecifier)) continue;
    imports.push({ path, module: node.moduleSpecifier.text });
  }
  return imports;
}
...
for (const file of files) {
  ...
  const fileSymbols = symbolsFrom(sourceFile, path);
  const fileImports = importsFrom(sourceFile, path);

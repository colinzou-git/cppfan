...
const indexPath = indexFlag >= 0 ? args[indexFlag + 1] : ".ai/code-index.json";
const limit = limitFlag >= 0 ? Number(args[limitFlag + 1]) : 25;

if (!query) {
  console.error("Usage: pnpm ai:query -- <text> [--limit N] [--index path]");
  process.exit(2);
}
...
try {
  ...
  console.error(`Code index not found at ${indexPath}. Run: pnpm ai:index`);
  process.exit(1);
}
...
for (const path of index.files ?? []) {
  if (path.toLowerCase().includes(needle)) {
    results.push({ score: 4, text: `file ${path}` });
  }
}
...
if (selected.length === 0) {
  console.log(`No code-index matches for: ${query}`);
} else {

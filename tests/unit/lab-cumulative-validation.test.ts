...
vi.mock("@/features/code-lab/code-runner", async () => {
  const actual = await vi.importActual<typeof import("@/features/code-lab/code-runner")>("@/features/code-lab/code-runner");
...
function milestonesPayload() {
  return {
    ...
    milestones: [
      { id: "m1", title: "Parse", instructions: "p", required: true, tests: [{ name: "t1", input: "", expectedOutput: "PASS\n", hidden: false }] },
      { id: "m2", title: "Run", instructions: "r", required: true, tests: [{ name: "t2", input: "", expectedOutput: "OTHER\n", hidden: false }] }
    ]
  };
}
...
describe("validateLabCumulative (#610)", () => {
  it("reports required milestones the current code no longer passes", async () => {
    getLabForOwner.mockResolvedValue({ publishedPayload: milestonesPayload(), publishedVersionId: "v1" });
    ...
    const result = await validateLabCumulative({ itemId: ITEM, contentVersionId: "v1", source: "int main(){}" });
  ...
  it("returns ok when the code passes every required milestone", async () => {
    ...
    executeRun.mockResolvedValue({ status: "success", stdout: "PASS\n" });
    expect((await validateLabCumulative({ itemId: ITEM, contentVersionId: "v1", source: "x" })).status).toBe("ok");
  });

  it("skips (does not block) when no runner is configured", async () => {
    getLabForOwner.mockResolvedValue({ publishedPayload: milestonesPayload(), publishedVersionId: "v1" });
    executeRun.mockResolvedValue({ status: "runner_unconfigured", stdout: "" });
    expect((await validateLabCumulative({ itemId: ITEM, contentVersionId: "v1", source: "x" })).status).toBe("skipped");
  });

  it("refuses a stale content version", async () => {
    getLabForOwner.mockResolvedValue({ publishedPayload: milestonesPayload(), publishedVersionId: "v2" });
  ...
  it("is unavailable for a non-user or unpublished item", async () => {
    expect((await validateLabCumulative({ itemId: "native.x", source: "x" })).status).toBe("unavailable");
